...
; (function ($) {
    ...
    var Mapplic = function (element) {
        ...
        self.init = function (options) {
            ...
            var dataJson = GetJsonFromSVG(self.o.source);
        ...
        // obtenemos el json a partir del svg dibujado
        function GetJsonFromSVG(idSvg) {
            var img = $("#" + idSvg).parent().find("img");

            $("#" + idSvg).attr("viewbox", "0 0 " + img.width() + " " + img.height());
            var dataJson =
                {
                    mapwidth: img.width(),
            ...
            $("#" + idSvg).find("rect").each(function (index) {
                var location =
                    {
                        title: $(this).attr("data-nombre"),
                        id: $(this).attr("data-id"),
                        about: $(this).attr("data-id"),
                        pin: "hidden",
                        // pin: "default",                               
                        fill: $(this).attr("fill"),
                        x: $(this).attr("x"),
                        y: $(this).attr("y"),
                        zoom: "3",
                        category: $(this).attr("data-nombre"),
                        action: "none"
                    };

                level1.locations.push(location);
            });

            dataJson.levels.push(level1);
        ...
        var processData = function (data) {
            ...
            if (data.levels) {
                $.each(data.levels, function (index, level) {
                    ...
                    $("#" + level.map).addClass('mapplic-map-image');
                    $("#" + level.map).wrap($('<div></div>').addClass('mapplic-layer').attr('data-floor', level.id).hide());
            ...
            self.map = self.el.find("#wrapMapa").parent().wrap($('<div></div>').addClass('mapplic-map')).parent();
            if (self.o.zoom) self.map.addClass('mapplic-zoomable');

            self.container = self.map.wrap($('<div></div>').addClass('mapplic-container')).parent();
        ...
        self.moveTo = function (x, y, s, duration, easing, ry) {
            ...
            var toNormX = self.container.width() * 0.5 - self.scale * self.contentWidth * x;
            var toNormY = self.container.height() * ry - self.scale * self.contentHeight * y;

            self.x = normalizeX(toNormX);
            self.y = normalizeY(toNormY);
